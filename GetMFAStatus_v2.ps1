function Get-MFAStatus
{
    ...
    $ExportCSV="$home\desktop\MFADisabledUserReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
    $ExportCSVReport="$home\desktop\MFAEnabledUserReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
    ...
    if((Test-Path -Path $ExportCSV) -eq "True")
    {
     ...
     If ($UserInput -eq 6)
     {
      Import-Csv "$ExportCSV" | Out-GridView
     }
    ...
    elseif((Test-Path -Path $ExportCSVReport) -eq "True")
    {
     ...
     If ($UserInput -eq 6)
     {
      Import-Csv "$ExportCSVReport" | Out-GridView
     }
     Write-Host Exported report has $PrintedUser users
    }
    Else
